...
public class ConfigTests {
    ...
    public void testConfig() {
        ...
        assertEquals(5, config.ASSASSIN_ATTACK);
        assertEquals(3, config.ASSASSIN_BRIBE_AMOUNT);
        assertEquals(2, config.ASSASSIN_BRIBE_FAIL_RATE);
        assertEquals(5, config.ASSASSIN_HEALTH);
        assertEquals(2, config.ASSASSIN_RECON_RADIUS);
        assertEquals(8, config.HYDRA_ATTACK);
        assertEquals(8, config.HYDRA_HEALTH);
        assertEquals(1, config.HYDRA_HEALTH_INCREASE_AMOUNT);
        assertEquals(5, config.MIND_CONTROL_DURATION);
        assertEquals(6, config.MIDNIGHT_ARMOUR_ATTACK);
        assertEquals(6, config.MIDNIGHT_ARMOUR_DEFENCE);
    }
    ...
    public void testDefaultConfig() {
        ...
        assertEquals(2, config.BRIBE_AMOUNT);
        assertEquals(3, config.BRIBE_RADIUS);
        ...
        assertEquals(5, config.MERCENARY_ATTACK);
        assertEquals(10, config.MERCENARY_HEALTH);
        assertEquals(10, config.PLAYER_ATTACK);
        assertEquals(10, config.PLAYER_HEALTH);
        ...
        assertEquals(2, config.SWORD_ATTACK);
        assertEquals(4, config.SWORD_DURABILITY);
        assertEquals(1, config.TREASURE_GOAL);
        assertEquals(5, config.ZOMBIE_ATTACK);
        assertEquals(5, config.ZOMBIE_HEALTH);
        assertEquals(3, config.ZOMBIE_SPAWN_RATE);
